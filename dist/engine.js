...
export class Engine {
    ...
    movePiece(dx, dy) {
        if (!this.state.currentPiece || this.state.isGameOver || this.state.isPaused)
            return false;
        const newPos = {
    ...
    checkCollision(pos, shape, grid = this.state.grid) {
    ...
    clearLines() {
        ...
        for (let y = GRID_HEIGHT - 1; y >= 0; y--) {
            if (this.state.grid[y].every((cell) => cell !== 0)) {
